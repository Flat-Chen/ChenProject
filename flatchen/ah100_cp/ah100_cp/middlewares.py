...
class ah100_cpProxyMiddleware(object):
    def __init__(self):
        self.count = 0
    ...
    def process_request(self, request, spider):
        if spider.name in ['an100_ceping']:
            # if self.count < 2:
            #     request.meta['proxy'] = self.proxy
            #     self.count += 1
            # else:
...
class ah100_cpUserAgentMiddleware(UserAgentMiddleware):
    def process_request(self, request, spider):
...
class Ah100CpSpiderMiddleware(object):
...
class Ah100CpDownloaderMiddleware(object):
    # Not all methods need to be defined. If a method is not defined,
