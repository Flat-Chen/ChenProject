...
def get_youxinpai_cookies():
    ...
    # 打开浏览器并访问指定地址
    driver = webdriver.PhantomJS(executable_path='/usr/bin/phantomjs', desired_capabilities=dcap)  # 构造网页驱动
    ...
    if len(cookies) < 5:
        ...
        redis_cli = redis.Redis('192.168.1.241', port=6379, db=0)
...
# url = "http://i.youxinpai.com/CorpAccount/CorpLinkManOrMessage.aspx"
