...
import json
import re
# from .items import JzgModellistSpider Item


class JzgModellistSpider(scrapy.Spider):
    name = 'jzg_modellist'
    # allowed_domains = ['jzg.com']
    # start_urls = ['http://jzg.com/']
    ...
    def __init__(self, **kwargs):
        super(JzgModellistSpider, self).__init__(**kwargs)
        self.counts = 0
    ...
    custom_debug_settings = {
        ...
        'MYSQL_DB': 'jzg',
        'MYSQL_TABLE': 'jzg_modellist',
        'MONGODB_SERVER': '192.168.1.94',
        'MONGODB_DB': 'jzg',
        'MONGODB_COLLECTION': 'jzg_modellist',
    ...
    def parse(self, response):
        ...
        for data in json_data:
            brandid = data["makeId"]
            brandname = data["makeName"]
            meta = {
                "brandid": brandid,
                "brandname": brandname
            }
            response.meta.update(**meta)
            series_url = f"http://m.jingzhengu.com/carStyle/getModelList?makeId={brandid}&isEst=1&produceStatus=0"
            yield scrapy.Request(
                url=series_url,
    ...
    def parse_series(self, response):
        json_data = json.loads(response.text)["data"]["manufacturerList"]
        for data in json_data:
            groupName = data["groupName"]
            series_list = data["list"]
            for series in series_list:
                ...
                response.meta.update(**meta)
                # http://m.jingzhengu.com/carStyle/getStyleList?modelId=10000034&isEst=1&produceStatus=0
                model_url = f"http://m.jingzhengu.com/carStyle/getStyleList?modelId={seriesid}&isEst=1&produceStatus=0"
                yield scrapy.Request(
                    url=model_url,
    ...
    def parse_model(self, response):
        json_data = json.loads(response.text)["data"]["yearGroupList"]
        for data in json_data:
            model_list = data["list"]
            for model in model_list:
                meta = dict()
                meta["modelid"] = model["id"]
                meta["modelname"] = model["name"] + ' ￥ ' + str(model["nowMsrp"])
                meta["makeYear"] = model["year"]
                meta["nextYear"] = model["nextYear"]
                # meta["nowMsrp"] = model["nowMsrp"]
                meta["model_full_name"] = model["fullName"]
                response.meta.update(**meta)
                detail_url = f"http://appraise.jingzhengu.com/buyAppraise/getCarStyleProperties?styleId={model['id']}"
                yield scrapy.Request(
                    url=detail_url,
    ...
    def parse_config(self, response):
        ...
        item["familyname"] = response.meta["seriesname"]
        ...
        item["make_year"] = response.meta["makeYear"]
        item["next_year"] = response.meta["nextYear"]
        ...
        item['factory'] = response.meta["groupName"]
        # print(item["factory"])
        yield item
