...
# class FeijiuSpider(scrapy.Spider):
    ...
    custom_debug_settings = {
        ...
        # 'MONGODB_PORT': 27017,
        'MONGODB_COLLECTION': 'feijiu_2ssb',
        'MONGODB_DB': 'baogang',
    ...
    def parse(self, response):
        ...
        # class_url_list = ["http://www.feijiu.net/ErShouSheBei/"]
        class_url_list = ["http://www.feijiu.net/ErShouSheBei/"]
        for url in class_url_list:
            yield scrapy.Request(
                url=url,
                callback=self.get_lable_one_url,
                dont_filter=True,
    ...
    def get_lable_one_url(self, response):
        ...
        for lable_one in lable_one_url_list:
            if '#' not in lable_one:
                yield scrapy.Request(
                    url=lable_one,
                    # callback=self.get_lable_two_url,
    ...
    #     lable_two_url_list = response.xpath("//div[@id='hy_sj']/div/a/@href").getall()
    #     for lable_two in lable_two_url_list:
    #         yield scrapy.Request(
    #             url=lable_two,
    #             callback=self.get_lable_three_url,
    #             headers=self.headers
    #         )
    #
    ...
    def parse_detail_url(self, response):
        ...
        item["info"] = response.xpath("//div[@class='p']/text()").get().replace(" ", "").replace("\r", "").replace("\t", "").replace("\n", "")
