...
# class OuyeelJjSpider(scrapy.Spider):
class OuyeelJjSpider(RedisSpider):
    name = website
    # allowed_domains = ['ouyeel.com']
    redis_key = "ouyeel_jj_bg:start_urls"
    ...
    custom_debug_settings = {
        ...
        'MONGODB_PORT': 27017,
        'MONGODB_DB': 'baogang',
        'MONGODB_COLLECTION': 'ouyeel_jj_new_bg',
    ...
    def __init__(self, **kwargs):
        super(OuyeelJjSpider, self).__init__(**kwargs)
    ...
    def parse(self, response):
        ...
        if len(list_data["data"]) != 0:
            for data in list_data["data"]:
                ...
                yield scrapy.Request(
                    url=app_detail_url,
                    callback=self.parse_app_jj_detail_url,
                    meta={"item": item},
                    dont_filter=True,
                )
    ...
    def parse_app_jj_detail_url(self, response):
        item = response.meta["item"]
        detail_data = json.loads(response.text)
        data_dic = detail_data["package"][0]
        for k, v in data_dic.items():
            if k in jj_field_list:
                item[k] = v
        item["_id"] = uuid.uuid4().__str__()
        item["grabtime"] = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
        yield item
