...
website ='new_autohome_newcar_v2'
...
class CarSpider(scrapy.Spider):
    ...
    def parse_fnf(self, response):
        ...
        for ps in pss:
            ...
            if ps.xpath("@class").extract_first() != 'current' and ps.xpath("@class").extract_first != 'disabled':
                yield scrapy.Request(url=response.urljoin(ps.xpath("a/@href").extract_first()), callback=self.parse_fnf2)

        next = response.xpath("//a[@class='page-item-next']")
        if next:
            yield scrapy.Request(url=response.urljoin(next.xpath('@href').extract_first()), callback=self.parse_fnf2)


        divs = response.xpath("//*[@id='brandtab-1']/div")
        # print(str(len(divs)) + response.url)
        for div in divs:
            if div.xpath("@class").extract_first() == 'brand-title':
                factoryname = div.xpath("a/h3/text()").extract_first()
                factoryid = div.xpath("span/div/@id").extract_first()
            elif div.xpath("@class").extract_first() == 'list-cont':
                familyname = div.xpath("div/div[2]/div[1]/a/text()").extract_first()
                familyid = div.xpath("@data-value").extract_first()
            elif div.xpath("@class").extract_first() == 'intervalcont fn-hide':
                ...
                for li in lis:
                    ...
                    url = "https://www.autohome.com.cn/spec/%s" % autohomeid
                    yield scrapy.Request(url=url, meta=dict(meta, **response.meta), callback=self.parse_extra)

        next = response.xpath("//a[@class='page-item-next']")
        if next:
            yield scrapy.Request(url=response.urljoin(next.xpath('@href').extract_first()),
                                 callback=self.parse_fnf2)

    def parse_fnf2(self, response):
        # print(response.body)
        pss = response.xpath("//*[@class='tab-nav border-t-no']/*[@data-trigger='click']/li")
        for ps in pss:
            if ps.xpath("@class").extract_first() == 'current':
    ...
    #     # print(str(len(lis)) + response.url)
    #     for li in lis:
    #         model = li.xpath("div/div/p/a/text()").extract_first()
    #         autohomeid = li.xpath("@data-value").extract_first()
    #         meta = {
    ...
    #             "autohomeid": autohomeid
    #         }
    #         url = "https://www.autohome.com.cn/spec/%s" % li.xpath("@data-value").extract_first()
    #         yield scrapy.Request(url=url, meta=dict(meta, **response.meta), callback=self.parse_extra)
