# -*- coding: utf-8 -*-
import scrapy
from ganji.items import AutohomeItem_price
import time
...
class CarSpider(scrapy.Spider):
    ...
    def parse_network(self,response):
        # logging.log(msg=str(response.xpath('//li[@class="list-item"]')) + "$$$$$$$$$$" + "parse_carinfo" + response.url,
        #             level=logging.INFO)
        for shop in response.xpath('//li[@class="list-item"]'):
            ...
            # print url
            yield scrapy.Request(url,meta={'citydata': response.meta['citydata'],'branddata': response.meta['branddata']},callback=self.parse_carinfo)
        ...
        if kindnext:
            if not (kindnext.find('kindId=2') == -1):
                ...
                # print url
                yield scrapy.Request(url,meta={'citydata': response.meta['citydata'],'branddata': response.meta['branddata']},callback=self.parse_network)
    ...
    def parse_carinfo(self, response):
        ...
        for tmp in  response.xpath('//div[@class="brandtree-cont"]/dl/dd'):
            ...
            yield scrapy.Request(contenturl,meta={'citydata': response.meta['citydata'],'branddata': response.meta['branddata']},callback=self.parse_netinfo)
    ...
    def parse_netinfo(self,response):
        ...
        item['factoryid'] = re.findall(r'\d+',response.xpath("//div[@class='carprice-title']/a/@href").extract_first())[1]
        #storeinfo
        ...
        for tmp in response.xpath('//dt[@class="fn-clear"]'):
            ...
            item['guidepricerange'] = ".".join(re.findall(r'\d+-?\d+',tmp.xpath('div[@class="name"]/p/span/text()').extract()[1].strip()))
