...
website ='new_autohome_newcar'

class CarSpider(scrapy.Spider):

    name = website
    allowed_domains = ["autohome.com.cn"]
    start_urls = ["https://car.autohome.com.cn/javascript/NewSpecCompare.js?20131010"]
    ...
    def parse(self, response):
        text = response.body.replace("var listCompare$100= ", "").decode("GB2312")
        brand_list = json.loads(text.strip()[:-1])
        car_info = {}
        for brand in brand_list:
            car_info["brandname"] = brand["N"]
            car_info["brandid"] = brand["I"]
            for factory in brand["List"]:
                car_info["factoryname"] = factory["N"]
                car_info["factoryid"] = factory["I"]
                for family in factory["List"]:
                    car_info["familyname"] = family["N"]
                    car_info["familyid"] = family["I"]
                    url = "https://car.autohome.com.cn/duibi/ashx/specComparehandler.ashx?callback=jsonpCallback&type=1&seriesid=%s&format=json" % family["I"]
                    yield scrapy.Request(url=url, meta=car_info, callback=self.parse_model)

    def parse_model(self, response):
        model_list = json.loads(response.body.replace("jsonpCallback(", "").strip()[:-1].decode("GB2312"))
        car_model_info = {}
        for type in model_list["List"]:
            car_model_info["producestatus"] = type["N"]
            for model in type["List"]:
                car_model_info["model"] = model["N"]
                car_model_info["autohomeid"] = model["I"]
                url = "https://www.autohome.com.cn/spec/%s" % model["I"]
                yield scrapy.Request(url=url, meta=dict(car_model_info, **response.meta), callback=self.parse_extra)
                # url = "http://car.m.autohome.com.cn/ashx/car/GetModelConfig.ashx?ids=%s" % model["I"]
                # yield scrapy.Request(url=url, meta=dict(car_model_info, **response.meta), callback=self.parse_details)
